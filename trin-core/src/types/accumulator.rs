...
impl MasterAccumulator {
    ...
    fn update_accumulator(
        ...
        if self.current_epoch.header_records.len() == EPOCH_SIZE {
            ...
            db.insert(epoch_accumulator_key, epoch_accumulator_value);
...
fn is_header_canonical(
    header: &Header,
    master: &MasterAccumulator,
    epoch_db: &HashMap<Vec<u8>, Vec<u8>>,
) -> anyhow::Result<()> {
    match master.header_in_current_epoch(&header.number) {
        true => {
            ...
            if rel_index > (master.current_epoch.header_records.len() as u64 - 1u64) {
                return Err(anyhow!("header ahead of accumulator"));
            };
            if master.current_epoch.header_records[rel_index as usize].block_hash == header.hash() {
                Ok(())
            } else {
                Err(anyhow!("header not part of canonical chain"))
            }
        }
        false => {
            ...
            let epoch_key: Vec<u8> =
                HistoryContentKey::EpochAccumulator(EpochAccumulatorKey { epoch_hash }).into();
            let raw_epoch_accumulator = epoch_db.get(&epoch_key).unwrap();
            ...
            match header_record.block_hash == header.hash() {
                true => Ok(()),
                false => Err(anyhow!("invalid match")),
            }
        }
    }
}
...
mod test {
    ...
    fn master_accumulator_update() {
        ...
        headers
            ...
            .for_each(|(header, expected_root)| {
                master_accumulator.update_accumulator(header, &mut db);
                assert_eq!(
    ...
    fn master_accumulator_verifies_valid_headers() {
        ...
        for height in headers_to_test {
            ...
            is_header_canonical(&header, &master_accumulator, &db).unwrap();
        }
    }
