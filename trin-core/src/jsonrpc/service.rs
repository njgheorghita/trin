...
fn serve_http_client(
    ...
    let body_offset = match req.parse(&received) {
        Ok(val) => match val {
            httparse::Status::Complete(offset) => offset,
            httparse::Status::Partial => panic!("fuck"),
        },
        Err(msg) => panic!("fuck"),
    };
    let body = received[body_offset..received.len()].to_vec();
    let http_request = match String::from_utf8(body) {
        Ok(val) => val,
        Err(msg) => panic!("fuck"),
    };
    let deser = serde_json::Deserializer::from_str(&http_request);
    for obj in deser.into_iter::<JsonRequest>() {
        let obj = obj.unwrap();
        let formatted_response = match obj.validate() {
...
fn dispatch_trin_request(obj: JsonRequest, endpoint: TrinEndpointKind, infura_url: &str, portal_tx: UnboundedSender<PortalJsonRpcRequest>) -> Result<String, String>{
    match endpoint {
        ...
        TrinEndpointKind::Discv5EndpointKind(_) => dispatch_portal_request(obj, endpoint, portal_tx),
        TrinEndpointKind::HistoryEndpointKind(_) => dispatch_portal_request(obj, endpoint, portal_tx),
    ...
    match resp_rx.blocking_recv().unwrap() {
        ...
        Err(msg) => {
            Err(json!({
                "jsonrpc": "2.0",
                "id": obj.id,
                "error": format!("Error while processing {}: {}", obj.method.to_string(), msg),
            })
            .to_string())
        }
    }
