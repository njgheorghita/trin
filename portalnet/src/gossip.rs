...
// we create a separate function for external jsonrpc bc we need sync and we don't want to block
// the accept offer flow, bla bla
pub async fn trace_propagate_gossip_cross_thread<TContentKey: OverlayContentKey>(
    ...
    //gossip_result.offered = interested_enrs.iter().map(|enr| enr.node_id()).collect();
    ...
    for enr in interested_enrs.into_iter() {
        ...
        let offer_request = Request::PopulatedOfferWithResult(PopulatedOfferWithResult {
            content_items: vec![(content_key.clone().into(), data.clone())],
            result_tx,
        });
        ...
        if let Err(err) = command_tx.send(OverlayCommand::Request(overlay_request)) {
            error!(error = %err, "Error sending OFFER message to service");
            gossip_result.offered.push(enr.node_id());
        }
        match rx.await {
            Ok(res) => match res {
                Ok(response) => match response {
                    Response::Accept(accept) => {
                        if accept.content_keys.is_zero() {
                            debug!("no content keys accepted");
                            gossip_result.offered.push(enr.node_id());
                            continue;
                        }
                    }
                ...
                Err(_) => gossip_result.offered.push(enr.node_id()),
            },
            Err(err) => {
                gossip_result.offered.push(enr.node_id());
                warn!("unable to gossip: {err:?}");
            }
        }
        if let Some(result) = result_rx.recv().await {
            if result {
                gossip_result.transferred.push(enr.node_id());
            }
        }
    }
    gossip_result
