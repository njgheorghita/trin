    ...
    <TContentKey as TryFrom<Vec<u8>>>::Error: Debug,
{
    ...
    fn handle_offer(
        ...
        tokio::spawn(async move {
            ...
            let content_values = portal_wire::decode_content_payload(data).unwrap();
    ...
    fn process_content(
        ...
        match content {
            Content::ConnectionId(id) => {
                ...
                debug!(
                    protocol = %self.protocol,
                    "Skipping processing for content connection ID {id}",
                );
                let utp = Arc::clone(&self.utp_socket);
                ...
                let content_keys: TContentKey = request.content_key.clone().try_into().unwrap();
                let content_keys = vec![content_keys];
                    ...
                    let mut stream = match utp.connect_with_cid(cid.clone(), UTP_CONN_CFG).await {
                        ...
                        Err(err) => {
                            ...
                            warn!(
                                ...
                                cid.recv,
                                peer = ?cid.peer.client(),
                                "Unable to establish uTP conn based on AcceptXXX",
                            );
                            return;
                        }
                    };
